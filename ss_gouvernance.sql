...
Quickstart:   Summit Sports - Zero to Snowflake - Governance with Snowflake Horizon
Version:      v2   
Author:       Elizabeth den Dulk
...
Step 1 - System Defined Roles and Privileges
 ...
 Before beginning to deploy Role Based Access Control (RBAC) for Summit Sports,
...
ALTER SESSION SET query_tag = '{"origin":"sf_sit-is","name":"ss_zts","version":{"major":1, "minor":1},"attributes":{"is_quickstart":1, "source":"sql", "vignette": "governance_with_horizon"}}';
...
CREATE OR REPLACE ROLE ss_test_role
    COMMENT = 'Test role for Summit Sports';
...
GRANT ALL ON WAREHOUSE ss_dev_wh TO ROLE sysadmin;


-- next we will grant only OPERATE and USAGE privileges to our Test Role
GRANT OPERATE, USAGE ON WAREHOUSE ss_dev_wh TO ROLE ss_test_role;
...
GRANT USAGE ON DATABASE ss_101 TO ROLE ss_test_role;

GRANT USAGE ON ALL SCHEMAS IN DATABASE ss_101 TO ROLE ss_test_role;
...
GRANT SELECT ON ALL TABLES IN SCHEMA ss_101.raw_customer TO ROLE ss_test_role;

GRANT SELECT ON ALL TABLES IN SCHEMA ss_101.raw_pos TO ROLE ss_test_role;

GRANT SELECT ON ALL VIEWS IN SCHEMA ss_101.analytics TO ROLE ss_test_role;
...
GRANT ROLE ss_test_role TO USER identifier($my_user_var);
...
CREATE OR REPLACE TAG tags.summit_pii
    ALLOWED_VALUES 'NAME', 'PHONE_NUMBER', 'EMAIL'
    COMMENT = 'Tag for PII, allowed values are: FIRST_NAME, PHONE_NUMBER, EMAIL';


-- with the Tags created, let's assign them to the relevant columns in our Customer Loyalty table
ALTER TABLE raw_customer.customer_loyalty
    MODIFY COLUMN 
    first_name SET TAG tags.summit_pii = 'NAME',
    last_name SET TAG tags.summit_pii = 'NAME',
    phone_number SET TAG tags.summit_pii = 'PHONE_NUMBER',
    e_mail SET TAG tags.summit_pii = 'EMAIL';
...
FROM TABLE(information_schema.tag_references_all_columns
    ('ss_101.raw_customer.customer_loyalty','table'));
...
CREATE OR REPLACE MASKING POLICY governance.summit_pii_string_mask AS (val STRING) RETURNS STRING ->
    CASE
        -- these active roles have access to unmasked values 
        WHEN CURRENT_ROLE() IN ('ACCOUNTADMIN','SYSADMIN')
            THEN val 
        -- if a column is tagged with summit_PII : PHONE_NUMBER 
        -- then mask everything but the first 3 digits   
        WHEN SYSTEM$GET_TAG_ON_CURRENT_COLUMN('TAGS.summit_PII') = 'PHONE_NUMBER'
            THEN CONCAT(LEFT(val,3), '-***-****')
        -- if a column is tagged with summit_PII : EMAIL  
        -- then mask everything before the @ sign  
        WHEN SYSTEM$GET_TAG_ON_CURRENT_COLUMN('TAGS.summit_PII') = 'EMAIL'
...
ALTER TAG tags.summit_pii SET
    MASKING POLICY governance.summit_pii_string_mask;
...
Step 5 - Aggregation Policies
 ...
 For Summit Sports and the Test role we have created, let's test an Aggregation Policy
...
CREATE OR REPLACE AGGREGATION POLICY governance.summit_order_test_aggregation_policy
...
ALTER TABLE raw_pos.order_header
    SET AGGREGATION POLICY governance.summit_order_test_aggregation_policy;
...
CREATE OR REPLACE PROJECTION POLICY governance.summit_customer_test_projection_policy
...
ALTER TABLE raw_customer.customer_loyalty
 ...
 SET PROJECTION POLICY governance.summit_customer_test_projection_policy;
...
Step 9 - Access History (Read and Writes)
 ...
 For Summit Sports, Access History is particularly important for Compliance, Auditing,
 and Governance.
 ...
 the Summit Sports environment, some of the queries below may not have results.
---------------------------------------------------------------------------------*/
...
WHERE object_name ILIKE 'ss_101.raw_%'
...
WHERE 1=1
    AND object_name ILIKE 'ss_101.raw_%'
...
Step 10 - Discovery with Snowflake Horizon - Universal Search
 ...
 Universal Search enables Summit Sports to easily find Account objects,
...
-- drop Test Role
DROP ROLE IF EXISTS ss_test_role;

-- unset our Masking Policies
ALTER TAG tags.summit_pii UNSET 
    MASKING POLICY governance.summit_pii_string_mask,
    MASKING POLICY governance.summit_pii_date_mask;
